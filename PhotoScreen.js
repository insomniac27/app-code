...
import React, { useState } from 'react';
import { TextInput, View, Text, Image, StyleSheet, Dimensions, ScrollView, TouchableOpacity, Modal, Alert, TouchableWithoutFeedback } from 'react-native';
...
//  };
//
//  // Handle adding to the wishlist, without stock checks
//  const handleAddToWishlist = async () => {
//    const user = auth().currentUser;
//    if (!user) {
//      Alert.alert('Error', 'You need to sign in to add items to your wishlist.');
...
//  };
//
//  // Handle order completion with stock validation
//  const handleOrderCompletion = () => {
//    const updatedQuantities = {
//      S: item.quantityS,
//      M: item.quantityM,
...
//  };
const handleOrderCompletion = () => {
  // Ensure the selectedSize exists in updatedQuantities
  const updatedQuantities = {
    S: item.quantityS,
    M: item.quantityM,
    L: item.quantityL,
  ...
  // Convert both values to numbers to avoid string comparison issues
  const availableQuantity = Number(updatedQuantities[selectedSize]);
  const orderedQuantity = Number(quantity);

  if (isNaN(availableQuantity) || isNaN(orderedQuantity)) {
    Alert.alert('Error', 'Invalid quantity or stock data.');
    return;
  }
  ...
  if (availableQuantity < orderedQuantity) {
    Alert.alert(
      'Insufficient Stock',
      `Only ${availableQuantity} items available for size ${selectedSize}`
    );
    return;
  }

  // Update the stock quantities after order completion
  updatedQuantities[selectedSize] -= orderedQuantity;
  item[`quantity${selectedSize}`] = updatedQuantities[selectedSize];

  Alert.alert('Order Successful', `You ordered ${orderedQuantity} items of size ${selectedSize}`);
};


  // Get the available quantity based on size
  const getAvailableQuantity = () => {
    ...
    return updatedQuantities[selectedSize];
  };
